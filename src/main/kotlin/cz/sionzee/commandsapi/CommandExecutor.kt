...
class CommandExecutor(private val m_command: Command) {
    ...
    fun execute(sender: CommandSender, args: Array<out String>, command: ICommand): Boolean {
        ...
        if(args.isNotEmpty()) {
            ...
            if(method == null) {
                val clazz : Class<*> = command.javaClass.declaredClasses.getByName(args[m_continueInArg]) ?: return true//TODO: Method & Class not found.
//                println(clazz.constructors.joinToString { it.name + "(" + it.parameters.joinToString { it.type.name ?: "null" } + ")" })
                ...
                val newInstance = clazz.constructors[0].newInstance(command) as Command
                ...
                if(method.parameterCount != methodArgs.size) {
                    ...
                    println("[CommandsAPI][TODO] Method Parameter Count != Entered Parameter Count (continues in executing)")
                }
...
//                            println("[CommandsAPI] Method requires a non-primitive type. (${parameter.type.name}) is enter paremeter string? ${String::class.java.isInstance(enteredArgument)}")
    ...
    private fun copyOfRange(startIndex : Int, endIndex : Int, array: Array<out String>) : Array<out String> {
...
private fun Array<out Class<*>>.getByName(className: String): Class<*>? {
    this.iterator().forEach {
        if(it.getAnnotationsByType(SubCommand::class.java).isNotEmpty() && Command::class.java.isAssignableFrom(it) && it.simpleName.equals(className, true))
            return it
    }
    return null
}
